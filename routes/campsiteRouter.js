...
//options are for preflight requests
...
.get(cors.cors, (req, res, next) => {
    ...
    .then(campsite => {
        if (campsite && campsite.comments.id(req.params.commentId)) {
            ...
            //two else because it checks for campsite first always
        } else if (!campsite) {
            err = new Error(`Campsite ${req.params.campsite} not found`);
            err.status = 404;
            return next(err);
        } else {
            err = new Error(`Comments ${req.params.comment} not found`);
            err.status = 404;
            return next(err);
        }
    })
